...
class db
{
     ...
     * @return
     */
    public static function delete( $table , $where = '' ,array $where_binds)
    {
        // where statement
     ...
     * @return
     */
    public static function select( $table , array $columns , $where = '', array $where_binds = array() )
    {
        // where statement
        $where_stmt = (strlen(trim($where)) < 1) ? '' : ' WHERE ' . $where ;
        // sql statement
        $sql = 'SELECT ' . implode(', ', $columns) . ' FROM ' . $table . $where_stmt;
        //return $sql;
        // create query
