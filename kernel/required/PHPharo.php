...
final class PHPharo
{
    ...
    public static function Start()
    {
        ...
        if (self::$Started)
            self::Error('PHPharo is already started');
        ...
        // set my headers
        header('X-Powered-By: PHPharo', true);

        // start the framework
        self::$Started = true;

        // register spl_autoload
        spl_autoload_register(array('self', 'LoadLibrary'));
        ...
        if (version_compare(PHP_VERSION, PHPHARO_PHP_VERSION) < 0)
            die(self::Error('PHPharo Requires PHP At Least (version ' . PHPHARO_PHP_VERSION .
                ') Yours is (version ' . PHP_VERSION . ')'));
    ...
    public static function With($classname)
    {
        if (class_exists($classname))
            ...
            echo self::Error('PHPHARO::WITH : Class (' . $classname . ') NOT FOUND');
    }
    ...
    public static function Globals($key = '', $value = '')
    {
        if (is_array($key)) {
            foreach ($key as $k => $v):
                (strlen(trim($k)) > 0) ? self::$Globals[$k] = $v : '';
            endforeach;
        } else
            (strlen(trim($key)) > 0) ? self::$Globals[$key] = $value : '';
    ...
    public static function Load($files)
    {
        ...
        $paths = (is_array($files)) ? $files : $args;
        if (!empty($paths)):
    ...
    public static function LoadLibrary($library)
    {
        ...
        foreach ($libraries as $library):
            ...
            $path3 = self::Env()->KERNEL_LIBS . basename($library) . '/' . $library . '.php';
            $path2 = self::Env()->WWW_LIBS . $library . '.php';
            $path4 = self::Env()->WWW_LIBS . basename($library) . '/' . $library . '.php';
            ...
            elseif(file_exists($path4))
                ...
                self::Error('PHPharo::LoadLibrary The Library (' . $library .
                    ') Not Found in any libraries folder');
        endforeach;
    }
    ...
    {
        $path1 = self::Env()->MODS . $Mod . '/' . basename($Mod) . '.php';
        $path2 = self::Env()->MODS . basename($Mod) . '.php';
        if(file_exists($path1))
            self::Load($path1);
        elseif(file_exists($path2))
            self::Load($path2);
     ...
     * @return url
     */
    public static function Assets($path = '', $basic = true)
    {
        
        return self::Env()->URL . 'www/'. (($basic) ? 'assets/' : '') . $path;
    }
    ...
    public static function Env()
    {
        ...
        $uri = (isset($_SERVER['USE_MOD_REWRITE'])) ? $url : $url . basename($_SERVER['SCRIPT_NAME']) .
            '/';
        $env = array(
            'URL' => $url,
            'URI' => $uri,
    ...
    public static function Error($err_str)
    {
        ...
        $e = '<div style="' . $css . '">
                An error occured <br /> 
                it`s &laquo;<i style="color:yellowgreen">' . $err_str .
            '</i>&raquo;
              </div>';
